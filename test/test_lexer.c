...
/*   Updated: 2022/12/06 14:04:04 by fsnelder      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
Test(token_verification, lexer) {
	...
	cr_assert(get_token(tokens, 1)->type == TOKEN_REDIRECT_OUT);
	cr_assert(get_token(tokens, 2)->type == TOKEN_REDIRECT_IN);
	ft_lstclear(&tokens, free);
}
...
Test(token_verification_operators, lexer) {
	...
	cr_assert(get_token(tokens, 1)->type == TOKEN_REDIRECT_HEREDOC);
	cr_assert(get_token(tokens, 2)->type == TOKEN_REDIRECT_APPEND);
	ft_lstclear(&tokens, free);
}
